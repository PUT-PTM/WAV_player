...
void play(FRESULT fresult, struct List *track)
{
  ...
  TIM_Cmd(TIM3, ENABLE);// w³¹czenie timera odp za aktualny czas
  ...
  while(change == 0)//je¿eli nie wcisniêto przycisku przewiniêcia w przód/cofniêcia
  {
	 ...
	 f_read (&file,&Buff[0],256,&ReadBytes);//wype³nienie 1 po³owy bufora

	 b=0;
	 while(b<256)
	 {
		 tp =volume*(float)Buff[b];//uzyskanie zadanej g³osnosci
		 Buff[b]=tp;
		 b++;
	 }
     ...
     f_read (&file,&Buff[256],256,&ReadBytes);//wype³nienie 2 po³owy bufora
